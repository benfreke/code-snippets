...
/**
 * MySqliDb Class
 * Amendments to the _dynamicBindResults function so it actually works
 *
...
class MysqliDB
{
    ...
    {        
        ...
        // For each field, we need to be able to bind it to the result
        while ($field = $meta->fetch_field()) { 
            $var = $field->name; 
            $$var = null; 
            $fields[$var] = &$$var;
        }
        // Now bind each value in the array to a result
        call_user_func_array(array($stmt, "bind_result"), $fields);
        
        // While we have some results
        while ($stmt->fetch()) {
            // I need the values from the fields, not the fields themselves
            foreach($fields as $key => $value) {
                $tmpArray[$key] = $value;
            }            
            // Make it an array, as I find it easier to write these
            $results[] = (object) $tmpArray;
        }
        return $results;
    }
